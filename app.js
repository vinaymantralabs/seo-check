...
async function check(url, resolve) {
    const scoreArray = [];
    let finalScore = 0;
    await crawler.load(url, async function (response) {
        ...
        for (const meta of rules.metaTagRules) {
            ...
            for (let property of meta.property) {
                ...
                if (property.type == "required") {
                    ...
                    if (metaTagsArray[meta.name] != null && metaTagsArray[meta.name].length >= property.minContentLength && metaTagsArray[meta.name].length <= property.maxContentLength) {
                        score = score + scoreProperty;
        ...
        if (url.includes("https://")) {
            scoreArray.push({
                ...
                name: "https",
                value: url,
                reasonPositive: ["Your site supports HTTPS. OK"],
                reasonNegative: []
            })
        } else {
            scoreArray.push({
                ...
                name: "https",
                value: url,
                reasonPositive: ["Your site does not supports HTTPS. KO"],
                reasonNegative: []
            })
        }

        // check page length
        let pageLength = cases.checkPageContent(body, url, metaTagsArray);
        scoreArray.push(pageLength);
        ...
        let resultArray = {finalScore : finalScore, scoreArray : scoreArray, title : metaTagsArray.title};
        resolve(resultArray);
        return resultArray;
